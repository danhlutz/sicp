...
;     (iter a b))
      ...
      (if (same-variable? (variable p1) (variable p2))
          (let ((reduced-terms 
                  (reduce-terms (term-list p1)
                                (term-list p2))))
            (newline)
            (display "reduced-poly-terms: ")
            (display reduced-terms)
    ...
    (define (reduce-terms n d)
      (define (simplify n d)
        ...
        (display " d: ")
        (display d)
        (let ((g (gcd-all-terms n d)))
          (newline)
          (display "simplify g: ")
          (display g)
          (let ((g-term (adjoin-term
                          (make-term 0 g)
                          (the-empty-termlist))))
            (newline)
            (display "g-term: ")
            (display g-term)
            (list (car (div-terms n g-term))
                  (car (div-terms d g-term))))))
      (define (gcd-all-terms n d)
        (newline)
        (display "gcd-all-terms n: ")
        (display n)
        (display " d: ")
        (display d)
        (newline)
        (display "coeff of first-term n ")
        (display (coeff (first-term n)))
        (let ((gcd-terms
                (gcd-nd-terms (make-list-coeffs n d))))
          (if (> (coeff (first-term n)) 0)
              gcd-terms
              (- gcd-terms))))
      (define (make-list-coeffs n d)
        (newline)
        (display "make-list-coeffs n: ")
        (display n)
        (display " d: ")
        (display d)
        (append (list-coeffs n) (list-coeffs d)))
      (define (list-coeffs L)
        (newline)
        (display "list-coeffs L: ")
        (display L)
        (if (empty-termlist? L)
            '()
            (cons (coeff (first-term L))
                  (list-coeffs (rest-terms L)))))
      (define (gcd-nd-terms n-d-list)
        (newline)
        (display "gcd-nd-terms: ")
        (display n-d-list)
        (if (null? n-d-list)
            0
            (gcd (car n-d-list)
                 (gcd-nd-terms (cdr n-d-list)))))

      (define (reduce-iter n d)
        (newline)
        (display "reduce-iter n: ")
        (display n)
        (display " d: ")
        (display d)
        (let ((g (gcd-terms n d)))
          (newline)
          (display "reduce-iter found gcd: ")
          (display g)
          (let ((ifact (expt (coeff (first-term g))
                             (- (+ 1
                                   (max (order (first-term n))
                                        (order (first-term d))))
                                (order (first-term g))))))
            (let ((scale (adjoin-term 
                           (make-term 0 ifact)
                           (the-empty-termlist))))
              (list (car (div-terms (mul-terms n scale) g))
                    (car (div-terms (mul-terms d scale) g)))))))
      (let ((unsimplified-list (reduce-iter n d)))
        (let ((nn (car unsimplified-list))
              (dd (cadr unsimplified-list)))
          (newline)
          (display "unsimplifed-list: ")
          (display unsimplified-list)
          (newline)
          (display "nn: ")
          (display nn)
          (newline)
  ...
  (put 'reduce '(polynomial polynomial) 
       (lambda (p1 p2)
         (let ((reduced-terms (reduce-poly p1 p2)))
           (list (tag (car reduced-terms))
                 (tag (cadr reduced-terms))))))
  'done)
