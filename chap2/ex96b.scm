...
;   (display L2)
  ...
  (define (pseudoremainder-terms L1 L2)
    (define (find-factor t1 t2)
      (let ((power (- (+ 1 (order t1)) (order t2))))
        (expt (coeff t2) power)))
    (define (make-factor-poly t1 t2)
      (let ((factor (find-factor t1 t2)))
        (adjoin-term
          (make-term 0 factor)
          (the-empty-termlist))))
    ...
    (let ((t1 (first-term L1))
      ...
      (if (> (order t2) (order t1))
          ...
          (let ((i-factor-poly (make-factor-poly t1 t2)))
            (cadr (div-terms (mul-terms i-factor-poly L1) L2))))))
      ...
      (if (empty-termlist? L)
          ...
          (let ((t1 (first-term L)))
            (adjoin-term
              (make-term (order t1)
                         (/ (coeff t1) g))
              (simplify-terms (rest-terms L) g)))))
    (define (gcd-coeffs terms-in-list-form)
      (if (null? terms-in-list-form)
          0
          (gcd (car terms-in-list-form)
               (gcd-coeffs (cdr terms-in-list-form)))))
    (define (terms-as-list L)
      (if (empty-termlist? L)
          '()
          (cons (coeff (first-term L))
                (terms-as-list (rest-terms L)))))
    (let ((big-gcd (iter a b)))
      (let ((terms-in-list-form (terms-as-list big-gcd)))
        (let ((g (gcd-coeffs terms-in-list-form)))
          (simplify-terms big-gcd g)))))
