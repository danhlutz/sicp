...
(define (make-register name)
  (let ((contents '*unassigned*)
        (tracing false))
    (define (trace-register old new)
      (if tracing
          (begin (newline)
                 (display (list 'REG name 'OLD '= old 'NEW '= new)))))
  ...
  (let ((pc (make-register 'pc))
   ...
   (define (add-to-assignment-table-INTERNAL! inst)
     (if (tagged-list? inst 'assign)
         ...
         'no-action))
   (define (print-and-reset-instruction-count)
      (begin
        (newline)
   ...
   (define (print-instruction inst)
     (if instruction-tracing 
         (begin (if (= (instruction-position inst) 0)
                ...
                (newline) 
                (display "[")
                (display (instruction-position inst))
                (display "] ")
                (display (instruction-text inst)))))
   (define (trace-on) 
     (set! instruction-tracing true)
     (begin (newline)
            (display "Instruction tracing ON"))
     'ok)
   (define (trace-off)
     (set! instruction-tracing false)
     (begin (newline)
            (display "Instruction tracing OFF"))
     'ok)
   ;; BREAKPOINT METHODS
   (define (at-breakpoint? inst breakpoint)
     (and (equal? (instruction-label inst) (car breakpoint))
          (= (instruction-position inst) (cadr breakpoint))))
   (define (in-breakpoints? inst breakpoints)
     (cond ((null? breakpoints) false)
           ((at-breakpoint? inst (car breakpoints)) true)
           (else (in-breakpoints? inst (cdr breakpoints)))))
   (define (set-breakpoint! label position)
     (set! breakpoints (cons (list label position)
                             breakpoints))
     'done)
   (define (cancel-breakpoint! label position)
     (set! breakpoints
       (delete (list label position) breakpoints))
     'done)
   (define (cancel-breakpoints!)
     (set! breakpoints '())
     'done)
   (let ((the-ops
            (list (list 'initialize-stack
                  ...
                  (list 'read read)
                  (list 'print-instructions
                        (lambda ()
                          (print-and-reset-instruction-count)))))
          (register-table
        ...
        (if (assoc name register-table)
            ...
            (set! register-table
                  (cons (list name (make-register name))
                        register-table)))
        'register-allocated)
      ;; LOOKUP MODIFIED TO ALLOCATE REGISTER IF NOT FOUND
      ...
      (define (execute)
        (let ((insts (get-contents pc)))
          (cond ((null? insts) 'done)
                ((in-breakpoints? (car insts) breakpoints)
                 (begin (set-breakpoint! 'NONE 0)
                        (newline)
                        (display "BREAKPOINT ")
                        (display 
                          (list (instruction-label (car insts))
                                (instruction-position (car insts))))))
                (else
                  (begin
                    ...
                    (execute))))))
      (define (execute-after-breakpoint)
        (let ((insts (get-contents pc)))
          (cond ((null? insts) 'done)
                (else
                  (begin
                    ...
                    (set! instruction-count (+ 1 instruction-count))
                    (execute))))))
      (define (trace-register name msg)
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'stack-statistics) (stack 'print-statistics))
              ...
              ((eq? message 'proceed) (execute-after-breakpoint))
              ((eq? message 'cancel-breakpoint!) cancel-breakpoint!)
              ((eq? message 'cancel-breakpoints!) (cancel-breakpoints!))
...
(define (set-breakpoint machine label position)
  ((machine 'set-breakpoint!) label position))

(define (proceed machine)
  (machine 'proceed))

(define (cancel-breakpoint machine label position)
  ((machine 'cancel-breakpoint!) label position))

(define (cancel-all-breakpoints machine)
  (machine 'cancel-breakpoints!))
  ...
  (make-machine
    ...
    '(start
      ...
      fact-done
       ...
       (perform (op print-instructions))
       (perform (op print-stack-statistics)))))

(define fib-machine
  (make-machine
    ;; NO REGISTERS INITIALLY ALLOCATED
    '()
    (list (list '< <) (list '- -) (list '+ +) (list 'print display))
    '(start
        (assign n (op read))
