...
(define (make-save inst machine the-stacks pc)
  (let ((reg (get-register machine
                           (stack-inst-reg-name inst)))
        (stack (cadr (assoc (stack-inst-reg-name inst)
                            the-stacks))))
    (lambda ()
      (push stack (get-contents reg))
      (advance-pc pc))))

(define (make-restore inst machine the-stacks pc)
  (let ((reg (get-register machine
                           (stack-inst-reg-name inst)))
        (stack (cadr (assoc (stack-inst-reg-name inst) 
                            the-stacks))))
