(define version 0.2)
(define description "Allow ops only on registers and constants")
...
(define (make-stack-entry value name)
  (cons value name))

(define (stack-entry-val entry) (car entry))
(define (stack-entry-name entry) (cdr entry))

(define (make-stack)
  (let ((s '()))
    (define (push x reg-name)
      (set! s (cons (make-stack-entry x reg-name) s)))
    (define (pop reg-name)
      (if (null? s)
          (error "Empty stack -- POP")
          (let ((top (car s)))
            (cond ((not (eq? reg-name (stack-entry-name top)))
                   (error "Must restore to same register -- STACK"
                          reg-name))
                  (else (set! s (cdr s))
                        (stack-entry-val top))))))
    (define (initialize)
...
(define simple
  (make-machine
    ...
    '(start
        (assign val (const 666))
      re-assign
        (save val)
        (assign val (const 3.5))
      test
        (restore val)
...
(define bad
  (make-machine
    ...
    '(start
      ...
      test
        (restore other)
        (assign val (op +) (reg val) (reg other))
        (perform (op print) (reg val)))))

;; BAD raises an error when it is run
