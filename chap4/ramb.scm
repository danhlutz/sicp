(define version "amb 0.1.1")
(define description "AMB evaluator with RAMB. Based on meta-0.3.scm")
  ...
  (let ((cprocs (map analyze (ramb-choices exp))))
    (lambda (env succeed fail)
      (define (try-next choices)
        (if (null? choices)
            (fail)
            ((pick-random choices) env
                                   succeed
                                   (lambda ()
                                     (try-next choices)))))
      (try-next cprocs))))

(define (pick-random items)
  (let ((num-items (length items)))
    (list-ref items (random num-items))))
