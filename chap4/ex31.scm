...
;; plan
;; change any delay to a thunk
;; when a thunk is evaluated, just pass exp
;; when a thunk is forced, use force-it 
...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((delay? exp) (list 'thunk (cdr exp) env))
        ((thunk? exp) exp)
        ((force? exp) (eval (force-it exp) env))
        ((application? exp)
...
(define (thunk-body thunk) (cadr thunk))
(define (thunk-env thunk) (caddr thunk))

(define (force-it obj)
  (if (thunk? obj)
      (actual-value (thunk-body thunk) (thunk-env thunk))
      obj))
