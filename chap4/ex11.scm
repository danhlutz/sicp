...
(define (get-variable frame-pair) (car frame-pair))
(define (get-value frame-pair) (cdr frame-pair))
(define (make-frame-pair var val) (cons var val))
...
(define (make-frame variables values)
  (if (null? variables)
      ...
      (let ((var (car variables))
            (val (car values)))
        (cons (make-frame-pair var val)
              (make-frame-variables (cdr variables)
                                    (cdr values))))))
...
(define (add-binding-to-frame! var val frame)
  (cons (make-frame-pair var val) frame))
